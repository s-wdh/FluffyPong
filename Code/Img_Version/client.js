...
(function (FluffyPong_Img) {
    ...
    FluffyPong_Img.fluffies = [];
    FluffyPong_Img.walls = [];
    FluffyPong_Img.wallHoles = [];
    let timer;
    let gmTimer;
        ...
        switch (selector) {
            ...
            case "fluffy": {
                if (namesent == true) {
                    ...
                    let x = (FluffyPong_Img.canvasWidth / 2);
                    let y = (FluffyPong_Img.canvasHeight / 2);
                    //overwrite them with the correct values ...
                    switch (fluffy.direction) {
                        case "top":
                            x = fluffy.position;
                            y = FluffyPong_Img.borderWidth + 1;
                            break;
                        case "right":
                            x = (FluffyPong_Img.canvasWidth - FluffyPong_Img.borderWidth - FluffyPong_Img.fluffyWidth - 1);
                            y = fluffy.position;
                            break;
                        case "bottom":
                            x = fluffy.position;
                            y = (FluffyPong_Img.canvasHeight - FluffyPong_Img.borderWidth - FluffyPong_Img.fluffyHeight - 1);
                            break;
                        case "left":
                            x = FluffyPong_Img.borderWidth + 1;
                            y = fluffy.position;
                            break;
                    }
                    // ... if those are to big for the sreen size, set it to the middle ...
                    if (x > FluffyPong_Img.canvasWidth - FluffyPong_Img.fluffyWidth) {
                        x = (FluffyPong_Img.canvasWidth / 2);
                    }
                    if (y > FluffyPong_Img.canvasHeight - FluffyPong_Img.fluffyHeight) {
                        y = (FluffyPong_Img.canvasHeight / 2);
                    }
                    //... and generate and draw the new fluffy at the position
                    let position = new FluffyPong_Img.Vector(x, y);
                    let newFluffy = new FluffyPong_Img.FluffyElement(position);
                    newFluffy.generateColor();
                    newFluffy.draw();
                    FluffyPong_Img.fluffies.unshift(newFluffy);
                }
                break;
            }
    ...
    FluffyPong_Img.sendFluffy = sendFluffy;
    //send the ranking to the server, when the timer is at 0
    function getRanking() {
        //console.log("ranking");
        const gameEndMessage = {
            name: name,
            fluffyAmount: FluffyPong_Img.fluffies.length
        };
        const textCarrier = {
            selector: "ranking",
    ...
    FluffyPong_Img.getRanking = getRanking;
    ...
    function createRankingTable(_ranking) {
        ...
        div.id = "rankingDiv";
        let width = FluffyPong_Img.canvasWidth - (2 * FluffyPong_Img.borderWidth) - 6;
        div.style.width = width.toString() + "px";
        div.style.margin = FluffyPong_Img.borderWidth.toString() + "px";
